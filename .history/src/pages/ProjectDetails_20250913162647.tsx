...
const getStatusColor = (status: string) => {
  switch (status) {
    ...
    case "Miss":
      return "bg-red-300 text-yellow-900"
    default:
...
export default function ProjectDetailPage() {
  ...
  const { tasks, fetchTasks, loadingTasks, i } = useTaskStore();
  ...
  return (
    <div className="min-h-screen bg-background pb-16 md:pb-0">
        ...
        <div className="max-w-7xl mx-auto">
              ...
              <div className="flex justify-between items-start">
                <div className="space-y-2">
                  <div className="flex items-center gap-4">
                    <h1 className="text-3xl font-bold text-foreground">{projectById?.name ?? ""}</h1>
                    <Badge className={getStatusColor(projectById?.status ?? "")}>{projectById?.status}</Badge>
                  </div>
                  <p className="text-muted-foreground">{projectById?.description}</p>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">{projectById?.num_done}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">{projectById?.num_pending}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold">{new Date(projectById?.final_deadline ?? "").toLocaleDateString()}</p>
                  </div>
                </div>
                <div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="calendar">
              <TaskCalendarView tasks={taskByProject} />
            </TabsContent>
      ...
      {/* New Task Modal */}
      <NewTaskModal project_id={id} isOpen={isTaskModalOpen} onClose={() => setIsTaskModalOpen(false)} />
    </div>
  )
}
