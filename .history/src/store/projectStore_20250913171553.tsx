...
export const useProjectStore = create<ProjectState>((set) => ({
    ...
    fetchProjects: async () => {
        set({ loadingProjects: true, errorProjects: null, initializedProjects: true});
        try {
    ...
    addProject: async (newProjectData) => {
        try {
            ...
            const addedProject = {
  ...data[0],
  created_local: new Date(data[0].created_at + "Z").toLocaleString("vi-VN", {
    ...
    updateProject: async (projectId, updatedData) => {
        try {
            const { created_local, ...cleanData } = updatedData;
            const { data, error } = await supabase
                .from('projects')
                .update(cleanData)
                .eq('id', projectId)
